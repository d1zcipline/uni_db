...
if (!isset($_SESSION['customer']['id_customer'])) {
  echo "<script>alert('Для добавления товара в корзину нужно войти в личный кабинет или зарегистрироваться')</script>";
  echo "<script>location.replace('../products.php')</script>";
} elseif (currentUserAddress() == false) {
  echo "<script>alert('Для добавления товара в корзину нужно добавить адрес в личном кабинете')</script>";
  ...
  $query = "INSERT INTO `cart` (`customer_id`, `keyboard_id`, `quantity`) VALUES (:customer_id, :keyboard_id, :quantity)";
